...
export const handler = async (
  ...
  try {
    ...
    const link = await findPricingPageLink(links, domain);
    ...
    if (link) {
      ...
      const chunks = splitIntoChunks(cleanedContent, chunkSize, overlapSize);
      ...
      // Save to CSV
      // const csvFilePath = `${domainWithoutLink}-services.csv`;
      // await saveServicesToCSV(services, csvFilePath);

      // // Upload to S3
      // const bucketName = 'testbucketzizo';
      // const s3Key = `${domainWithoutLink}-services.csv`;
      // s3Link = await uploadFileToS3(csvFilePath, bucketName, s3Key);
